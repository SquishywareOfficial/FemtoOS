...
FemtoCasinoGame::FemtoCasinoGame(uint32_t width, uint32_t height)
    : App("Femto Casino", width, height) {}

bool FemtoCasinoGame::hasCustomOverlay() const {
  return true;
}

uint16_t FemtoCasinoGame::runningRenderIntervalMs() const {
  return 50;
}

uint16_t FemtoCasinoGame::staticRenderIntervalMs() const {
  return 1000;
}

void FemtoCasinoGame::ensureLoaded() {
  if (loaded_) {
    return;
  }
...
void FemtoCasinoGame::saveBankroll() {
  if (!loaded_) {
    return;
  }
...
uint16_t FemtoCasinoGame::playerInitials() const {
  return PlayerProfile::loadInitials();
}

void FemtoCasinoGame::onAppReset() {
  ensureLoaded();
  logic_.resetToHub();
}

void FemtoCasinoGame::onAppExit() {
  saveBankroll();
}

void FemtoCasinoGame::updateRunning(uint32_t deltaMs, const ButtonInput& b1, const ButtonInput& b2) {
  ensureLoaded();
...
void FemtoCasinoGame::drawShell(Canvas& canvas, const char* title) {
...
template <typename Canvas>
void FemtoCasinoGame::drawCard(Canvas& canvas, int x, int y, uint8_t card, bool hidden) {
...
template <typename Canvas>
void FemtoCasinoGame::drawSuit(Canvas& canvas, int x, int y, uint8_t suit) {
  const uint16_t color = suit == 1 || suit == 2 ? TFT_RED : TFT_BLACK;
  if (suit == 1) {
...
void FemtoCasinoGame::drawStart(TFT_eSPI& tft) {
  ensureLoaded();
  TDisplayFramebuffer::draw(tft, width, height, [&](auto& canvas) {
    drawShell(canvas, "Femto Casino");
...
void FemtoCasinoGame::drawRunning(TFT_eSPI& tft) {
  ensureLoaded();
  TDisplayFramebuffer::draw(tft, width, height, [&](auto& canvas) {
...
template <typename Canvas>
void FemtoCasinoGame::drawHub(Canvas& canvas) {
  drawShell(canvas, "Femto Casino");
...
template <typename Canvas>
void FemtoCasinoGame::drawBlackjack(Canvas& canvas) {
...
template <typename Canvas>
void FemtoCasinoGame::drawRoulette(Canvas& canvas) {
...
template <typename Canvas>
void FemtoCasinoGame::drawSlots(Canvas& canvas) {
...
template <typename Canvas>
void FemtoCasinoGame::drawHoldem(Canvas& canvas) {
...
template <typename Canvas>
void FemtoCasinoGame::drawVideoPoker(Canvas& canvas) {
...
template <typename Canvas>
void FemtoCasinoGame::drawBroke(Canvas& canvas) {
...
template <typename Canvas>
void FemtoCasinoGame::drawCashOut(Canvas& canvas) {
...
void FemtoCasinoGame::drawEnd(TFT_eSPI& tft) {
  ensureLoaded();
  TDisplayFramebuffer::draw(tft, width, height, [&](auto& canvas) {
    drawShell(canvas, "Femto Casino");
