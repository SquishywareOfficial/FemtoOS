...
CreditsGame::CreditsGame(uint32_t width, uint32_t height)
    : Game("Credits", width, height) {}

bool CreditsGame::hasCustomOverlay() const {
  return true;
}

void CreditsGame::onGameReset() {
  page_ = 0;
}

void CreditsGame::updateRunning(uint32_t deltaMs, const ButtonInput& input) {
  (void)deltaMs;
  if (input.longPress) {
    requestExitToMenu();
    return;
...
void CreditsGame::drawRunning(U8G2& u8g2) {
...
void CreditsGame::drawStart(U8G2& u8g2) {
  u8g2.drawFrame(0, 0, width + 2, height);
  u8g2.setFont(u8g2_font_5x8_tr);
...
void CreditsGame::drawEnd(U8G2& u8g2) {
  u8g2.drawFrame(0, 0, width + 2, height);
  u8g2.setFont(u8g2_font_5x8_tr);
  u8g2.drawStr(3, 12, "Thanks");
  u8g2.drawStr(3, 26, "Tap retry");
  u8g2.setFont(u8g2_font_4x6_tr);
