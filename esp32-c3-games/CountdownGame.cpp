...
const uint32_t CountdownGame::DURATIONS_MS[] = {
    ...
    60 * 60 * 1000};
const uint8_t CountdownGame::DURATION_COUNT = sizeof(CountdownGame::DURATIONS_MS) / sizeof(CountdownGame::DURATIONS_MS[0]);

CountdownGame::CountdownGame(uint32_t width, uint32_t height, uint32_t left)
    : Game("Countdown", width, height), durationMs_(DURATIONS_MS[selectedIndex_]), remainingMs_(durationMs_) {}

void CountdownGame::onGameReset() {
...
void CountdownGame::updateRunning(uint32_t deltaMs, const ButtonInput& input) {
  switch (mode_) {
    case Mode::Select:
      if (input.click) {
        // cycle selection
...
void CountdownGame::drawStart(U8G2& u8g2) {
  // Selection screen
...
void CountdownGame::drawRunning(U8G2& u8g2) {
...
bool CountdownGame::hasCustomOverlay() const {
  return true;
}
