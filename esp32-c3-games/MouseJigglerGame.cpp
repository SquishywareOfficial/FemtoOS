...
MouseJigglerGame::MouseJigglerGame(uint32_t width, uint32_t height, uint32_t left)
    : Game("Mouse Emulator", width, height), targetIntervalMs_(0), lastJiggleTime_(0), lastClockUpdate_(0), jigglerEnabled_(true), isMoving_(false) {
  (void)left;
}

void MouseJigglerGame::onGameReset() {
...
void MouseJigglerGame::updateRunning(uint32_t deltaMs, const ButtonInput& input) {
  (void)deltaMs;
...
void MouseJigglerGame::sendHumanizedStep() {
  int y = 0;
...
void MouseJigglerGame::drawRunning(U8G2& u8g2) {
...
void MouseJigglerGame::drawStart(U8G2& u8g2) {
  u8g2.drawFrame(0, 0, width + 2, height);
...
bool MouseJigglerGame::hasCustomOverlay() const {
  return true;
}
