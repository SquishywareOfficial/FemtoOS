...
StopwatchGame::StopwatchGame(uint32_t width, uint32_t height, uint32_t left)
    : Game("Stopwatch", width, height), running_(false), elapsedMs_(0) {}

void StopwatchGame::onGameReset() {
  running_ = false;
  elapsedMs_ = 0;
}

void StopwatchGame::updateRunning(uint32_t deltaMs, const ButtonInput& input) {
  if (input.click) {
    running_ = !running_;
  }
...
void StopwatchGame::drawRunning(U8G2& u8g2) {
...
void StopwatchGame::drawStart(U8G2& u8g2) {
...
bool StopwatchGame::hasCustomOverlay() const {
  return true;
}
