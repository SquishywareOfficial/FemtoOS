...
InitialsGame::InitialsGame(uint32_t width, uint32_t height)
    : Game("Options", width, height) {}

bool InitialsGame::hasCustomOverlay() const {
  return true;
}

void InitialsGame::onGameReset() {
  PlayerProfile::unpackInitials(PlayerProfile::loadInitials(), initials_);
  selected_ = 0;
}

void InitialsGame::updateRunning(uint32_t deltaMs, const ButtonInput& input) {
  (void)deltaMs;
  if (input.click) {
...
void InitialsGame::drawStart(U8G2& u8g2) {
  char dotted[4];
...
void InitialsGame::drawRunning(U8G2& u8g2) {
  u8g2.drawFrame(0, 0, width + 2, height);
  u8g2.setFont(u8g2_font_5x8_tr);
...
void InitialsGame::drawEnd(U8G2& u8g2) {
  u8g2.drawFrame(0, 0, width + 2, height);
  u8g2.setFont(u8g2_font_5x8_tr);
...
char InitialsGame::nextInitial(char value) const {
