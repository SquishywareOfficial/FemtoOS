...
CounterGame::CounterGame(uint32_t width, uint32_t height, uint32_t left)
    : Game("Counter", width, height), count_(0) {}

void CounterGame::onGameReset() {
  count_ = 0;
}

void CounterGame::updateRunning(uint32_t deltaMs, const ButtonInput& input) {
  (void)deltaMs;
  if (input.click) {
    // increment on tap
    count_++;
...
void CounterGame::drawRunning(U8G2& u8g2) {
...
void CounterGame::drawStart(U8G2& u8g2) {
  u8g2.drawFrame(0, 0, width + 2, height);
...
bool CounterGame::hasCustomOverlay() const {
  return true;
}
